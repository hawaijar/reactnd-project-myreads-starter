...
export default class SearchBook extends React.Component {
  ...
  createMatchedBook = (book: BookType, index: number) => {
    ...
    if (book.shelf === Constants.categories.CURRENT[0]) {
      return (
        <li key={index}>
          <Book book={book} updateBook={updateBook} shelf={Constants.categories.CURRENT[1]} />
        </li>
      );
    } else if (book.shelf === Constants.categories.WISH[0]) {
      return (
        <li key={index}>
          <Book book={book} updateBook={updateBook} shelf={Constants.categories.WISH[1]} />
        </li>
      );
    } else if (book.shelf === Constants.categories.READ[0]) {
      return (
        <li key={index}>
          <Book book={book} updateBook={updateBook} shelf={Constants.categories.READ[1]} />
        </li>
      );
    } else if (book.shelf === Constants.categories.NONE[0]) {
      return (
        <li key={index}>
          <Book book={book} updateBook={updateBook} shelf={Constants.categories.NONE[1]} />
        </li>
      );
    }
  };
